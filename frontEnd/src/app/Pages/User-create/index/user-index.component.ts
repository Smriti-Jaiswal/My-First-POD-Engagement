...
export class UserIndexComponent implements AfterViewInit, OnInit {
  displayedColumns: string[] = ['edit', 'firstName', 'email', 'gender', 'accountNumber', 'delete'];
  ...
  goToEdit(element: any): void {
    this.router.navigate(['/pages/users/edit/' + element.id])
  }


  removeOne(element: any) {
    let isConfirm = window.confirm("Are you sure want to delete ?");
    if (isConfirm) {
      this.utilService.DeleteOneUser(element.id)
        .subscribe((resp) => {
          this.toaster.open("delete Successfully", '', {
            duration: 5000,
